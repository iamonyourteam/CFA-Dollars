...
const buildDefaultUsers = () => {
  const allEmps = [...POSITIONS.Front, ...POSITIONS.Back];
  return {
    // Super-admins: can change manager PINs (and employee PINs)
    admins: [
      { name: "Admin 1", pin: "9999" },
    ],
    // Managers: can change employee PINs only
    managers: [
      { name: "Manager 1", pin: "1111" },
      { name: "Manager 2", pin: "2222" },
      { name: "Manager 3", pin: "3333" },
    ],
    employees: allEmps.map((name, i) => ({
      name,
      pin: String(4000 + i + 1),
...
const SEED_LOGS = [
  { id: 1, date: "2025-11-05", employee: "Adirana", task: "Cleaning list completed weekly", amount: 10, notes: "", enteredBy: "Manager 1", position: "Front" },
  { id: 2, date: "2025-11-06", employee: "Annika", task: "Cleaning list completed weekly", amount: 10, notes: "", enteredBy: "Manager 1", position: "Front" },
  { id: 3, date: "2025-11-13", employee: "Maria", task: "Cleaning list completed weekly", amount: -20, notes: "Did not complete", enteredBy: "Manager 2", position: "Back" },
  { id: 4, date: "2025-11-04", employee: "Haley", task: "Ecosure visit score = 1", amount: 100, notes: "Perfect score!", enteredBy: "Manager 1", position: "Front" },
];
...
export default function App() {
  const [users, setUsers] = useState(() => {
    // Version bump forces a clean rebuild of the users structure
    const DATA_VERSION = "v3";
    const storedVersion = load("cfa_users_version", null);
    if (storedVersion !== DATA_VERSION) {
      // Wipe old user data and start fresh with correct structure
      save("cfa_users_version", DATA_VERSION);
      save("cfa_users2", null);
      return buildDefaultUsers();
    }
    const loaded = load("cfa_users2", buildDefaultUsers());
    if (!loaded.managers) loaded.managers = buildDefaultUsers().managers;
    if (!loaded.admins || loaded.admins.length === 0) loaded.admins = buildDefaultUsers().admins;
    return loaded;
  });
  const [logs, setLogs] = useState(() => load("cfa_logs2", SEED_LOGS));
  const [session, setSession] = useState(null);
  const [screen, setScreen] = useState("login");

  useEffect(() => { save("cfa_logs2", logs); }, [logs]);
  useEffect(() => { save("cfa_users2", users); }, [users]);
  ...
  return (
    <AdminShell session={session} screen={screen} setScreen={setScreen} onLogout={logout}>
      ...
      {screen === "add-entry"   && <AddEntry logs={logs} setLogs={setLogs} session={session} employees={users.employees} onDone={() => setScreen("dashboard")} />}
      {screen === "history"     && <History logs={logs} setLogs={setLogs} employees={users.employees} />}
      {screen === "leaderboard" && <Leaderboard totals={totals} users={users} />}
      {screen === "manage-pins" && <ManagePins users={users} setUsers={setUsers} session={session} />}
    </AdminShell>
  );
}
...
function LoginScreen({ users, onLogin }) {
  ...
  function tryLogin(finalPin) {
    const found = allUsers.find(u => u.pin === finalPin);
    if (!found) {
      setShake(true);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
          {[1,2,3,4,5,6,7,8,9,null,0,"⌫"].map((d, i) => (
            <button key={i} onClick={() => {
              if (d === "⌫") backspace();
              else if (d !== null) pressDigit(String(d));
            }}
              style={{
                padding: "16px 0", fontSize: 22, fontWeight: "bold", borderRadius: 12, border: "none",
...
function Dashboard({ logs, totals, users, setScreen }) {
  ...
  const recent = [...logs].sort((a,b) => b.date.localeCompare(a.date)).slice(0, 8);
...
// ── ADD ENTRY (with batch selection) ─────────────────────────────────────────
function AddEntry({ logs, setLogs, session, employees, onDone }) {
  ...
  function selectAll() { setSelectedEmps(filteredEmps.map(e => e.name)); }
  function clearAll() { setSelectedEmps([]); }

  function handleRubricSelect(r) {
    setSelectedRubric(r);
    setCustomTask("");
  ...
  function submit() {
    if (selectedEmps.length === 0 || (!selectedRubric && !customTask.trim()) || !amount) return;
    const newLogs = selectedEmps.map(name => ({
  ...
  const canSubmit = selectedEmps.length > 0 && (selectedRubric || customTask.trim()) && amount;
    ...
    <div>
      ...
      {success > 0 && (
        <div style={{ background: "#dcfce7", border: "1px solid #86efac", borderRadius: 10, padding: 14, marginBottom: 20, color: "#166534", fontWeight: "bold", textAlign: "center" }}>
          ✅ Added entry for {success} employee{success > 1 ? "s" : ""}!
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 14, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          <div style={{ fontWeight: "bold", color: "#333", marginBottom: 14, fontSize: 15, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {selectedEmps.length > 0 && (
              <span style={{ background: "#E51636", color: "#fff", borderRadius: 20, padding: "2px 10px", fontSize: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button onClick={selectAll}
              style={{ flex: 1, padding: "7px", background: "#fee2e2", border: "none", borderRadius: 8, cursor: "pointer", color: "#E51636", fontWeight: "bold", fontSize: 12, fontFamily: "'Georgia', serif" }}>
              Select All ({filteredEmps.length})
            </button>
            <button onClick={clearAll}
              style={{ flex: 1, padding: "7px", background: "#f5f5f5", border: "none", borderRadius: 8, cursor: "pointer", color: "#666", fontSize: 12, fontFamily: "'Georgia', serif" }}>
              Clear
            </button>
          </div>
              ...
              return (
                <button key={e.name} onClick={() => toggleEmp(e.name)}
                  style={{ padding: "7px 13px", borderRadius: 20,
                    border: `2px solid ${sel ? "#E51636" : "#e5e5e5"}`,
        ...
        <div style={{ background: "#fff", borderRadius: 14, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              <div key={cat} style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                  {RUBRIC.filter(r => r.category === cat).map((r, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => handleRubricSelect(r)}
          ...
          <input placeholder="Custom description..." value={customTask}
            onChange={e => { setCustomTask(e.target.value); setSelectedRubric(null); }}
            style={inp} />
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any context..." rows={2}
          ...
          {selectedEmps.length > 0 && amount && (
            <div style={{ background: "#f9fafb", borderRadius: 8, padding: "10px 14px", marginBottom: 10, fontSize: 13, color: "#555", border: "1px solid #e5e5e5" }}>
              ...
              <strong style={{ color: Number(amount) >= 0 ? "#16a34a" : "#E51636" }}>
          ...
          <button onClick={submit} disabled={!canSubmit} style={canSubmit ? primaryBtn : disabledBtn}>
            {selectedEmps.length > 1 ? `➕ Add to ${selectedEmps.length} Employees` : "➕ Add Entry"}
          </button>
        </div>
      </div>
    </div>
...
// ── HISTORY ───────────────────────────────────────────────────────────────────
function History({ logs, setLogs, employees }) {
  const [filterEmp, setFilterEmp] = useState("");
  const [search, setSearch] = useState("");
  const [confirmId, setConfirmId] = useState(null);

  const filtered = [...logs]
    .filter(l => (!filterEmp || l.employee === filterEmp) &&
      (!search || l.task.toLowerCase().includes(search.toLowerCase()) || l.employee.toLowerCase().includes(search.toLowerCase())))
    .sort((a, b) => b.date.localeCompare(a.date));

  function del(id) { setLogs(prev => prev.filter(l => l.id !== id)); setConfirmId(null); }
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => setConfirmId(confirmId === l.id ? null : l.id)}
                    style={{ background: confirmId === l.id ? "#fee2e2" : "none", border: "none", cursor: "pointer",
                      color: confirmId === l.id ? "#E51636" : "#ccc", fontSize: 17, borderRadius: 6, padding: "4px 6px", transition: "all 0.15s" }}>
                    🗑
                  </button>
                </div>
              </div>
                ...
                <div style={{ marginTop: 10, padding: "10px 14px", background: "#fff5f5", borderRadius: 8, border: "1px solid #fecaca", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => setConfirmId(null)}
                      style={{ background: "#f5f5f5", border: "none", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 13 }}>
                      Cancel
                    </button>
                  </div>
                </div>
...
function Leaderboard({ totals, users }) {
  ...
  const allEmp = users.employees.map(e => ({
    name: e.name, position: e.position, total: totals[e.name] || 0,
...
// ── MANAGE PINs ───────────────────────────────────────────────────────────────
function ManagePins({ users, setUsers, session }) {
  // admin  → can see/edit: managers tab + employees tab
  // manager → can see/edit: employees tab only
  const canEditManagers = session.role === "admin";
  const defaultTab = canEditManagers ? "managers" : "employees";
  ...
  function startEdit(type, index) {
    setEditing({ type, index, newPin: "" });
    setPinError("");
  }

  function savePin() {
    if (!editing || editing.newPin.length !== 4 || !/^\d{4}$/.test(editing.newPin)) {
      setPinError("PIN must be exactly 4 digits.");
      return;
    }
    const all = [...users.admins, ...(users.managers || []), ...users.employees];
    // figure out global index for self-exclusion
    const offsets = { admins: 0, managers: users.admins.length, employees: users.admins.length + (users.managers || []).length };
    const selfGlobal = offsets[editing.type] + editing.index;
    const conflict = all.find((u, gi) => gi !== selfGlobal && u.pin === editing.newPin);
    if (conflict) { setPinError(`PIN already used by ${conflict.name}.`); return; }

    setUsers(prev => {
      const arr = [...(prev[editing.type] || [])];
      arr[editing.index] = { ...arr[editing.index], pin: editing.newPin };
      return { ...prev, [editing.type]: arr };
    });
  ...
  const listMap = { managers: users.managers || [], employees: users.employees };
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff3cd", border: "1px solid #ffc107", borderRadius: 10, padding: "10px 16px", marginBottom: 20, fontSize: 13, color: "#856404" }}>
        {canEditManagers
          ? "As an Admin, you can change Manager PINs and Employee PINs."
          : "As a Manager, you can change Employee PINs only. Admin PINs are managed by the Admin."}
      </div>
          ...
          return (
            <div key={u.name} style={{ padding: "13px 20px", borderBottom: "1px solid #f5f5f5", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <div>
                ...
                {tab === "managers" && <div style={{ fontSize: 12, color: "#888", background: "#fee2e2", display: "inline-block", padding: "1px 8px", borderRadius: 8, marginTop: 2 }}>Manager</div>}
              </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <input type="text" maxLength={4} value={editing.newPin} placeholder="New PIN"
                        onChange={e => { setEditing(prev => ({ ...prev, newPin: e.target.value.replace(/\D/g,"").slice(0,4) })); setPinError(""); }}
                        style={{ width: 80, padding: "7px 10px", border: "2px solid #E51636", borderRadius: 8, fontSize: 18, textAlign: "center", fontFamily: "monospace", outline: "none", letterSpacing: 4 }}
                        autoFocus />
                      <button onClick={savePin} style={{ background: "#E51636", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", cursor: "pointer", fontWeight: "bold", fontSize: 13 }}>Save</button>
                      <button onClick={() => { setEditing(null); setPinError(""); }} style={{ background: "#f5f5f5", border: "none", borderRadius: 8, padding: "7px 12px", cursor: "pointer", fontSize: 13 }}>✕</button>
                    </div>
      ...
      {/* Admin-only: show read-only admin section so they know it exists */}
      {canEditManagers && (
        <div style={{ marginTop: 24 }}>
          <div style={{ fontSize: 12, color: "#aaa", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Admin Accounts (your tier — PINs not editable here)</div>
          ...
          <div style={{ fontSize: 12, color: "#bbb", marginTop: 8 }}>To change Admin PINs, edit them directly in the app source or ask your system administrator.</div>
        </div>
      )}
    </div>
  );
